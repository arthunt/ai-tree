...
// Topic keywords for matching (i18n keys are in messages files)
const TOPIC_KEYWORDS: { topic: string; keywords: string[] }[] = [
    { topic: 'neural', keywords: ['neural', 'network', 'neuron', 'layer'] },
    { topic: 'tokenization', keywords: ['token', 'tokeniz', 'word', 'text', 'split'] },
    { topic: 'temperature', keywords: ['temperature', 'random', 'creative', 'deterministic'] },
    { topic: 'training', keywords: ['train', 'learning', 'gradient', 'backprop', 'epoch', 'loss'] },
    { topic: 'prompting', keywords: ['prompt', 'instruction', 'context', 'system'] },
    { topic: 'attention', keywords: ['attention', 'transformer', 'self-attention'] }
];

// Context-aware mock LLM response generator with i18n support
const generateMockResponse = (
    ...
    const matched = TOPIC_KEYWORDS.find(({ keywords }) =>
        keywords.some(k => lower.includes(k))
    );

    // Select temperature level
    const level = temperature < 0.3 ? 'low' : temperature < 0.8 ? 'mid' : 'high';

    if (matched) {
        // Use translated topic response
        return t(`${matched.topic}.${level}`);
    }

    // Fallback - alternate between two generic responses
    const fallbackNum = (prompt.length % 2) + 1;
    return t(`fallback${fallbackNum}.${level}`);
};
...
export function PromptSandbox({
    ...
    const tMock = useTranslations('sapling.mockResponses');
    ...
    const handleSend = async () => {
        ...
        setTimeout(() => {
            const botMsg: Message = {
                ...
                role: 'assistant',
                content: generateMockResponse(userMsg.content, temperature, tMock),
