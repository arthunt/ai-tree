...
INSERT INTO node_translations (node_id, locale, display_name, description, significance, metaphor) VALUES
  ...
  ('algorithm', 'et',
   ...
   'Iga tehisintellekti süsteem, ELIZAst kuni GPT-4-ni, põhineb sellel fundamentaalsel ideel: reeglid, mis muudavad sisendi väljundiks.',
   'Üherakuline organism, millest kogu tehisintellekti elu arenes.'),

  -- Eras
  ('era-symbolic', 'et',
   'Sümbolistlik tehisintellekt',
   'Esimene suur katse: õpetada masinatele mõeldud käsitsi kodeeritud reeglite ja loogika abil. Domineeris AI uurimises 1956–1980ndatel.',
   'Tõestas, et masinad võivad arutleda konkreetsete valdkondade üle, kuid ei suutnud toime tulla tegeliku maailma segadusega.',
   'Tehisintellekti dinosaurused — kunagi domineerivad, nüüd enamasti välja surnud.'),

  ('era-neural', 'et',
   'Närvivõrgud',
   'Ajust inspireeritud süsteemid, mis õpivad andmetest, mitte ei järgi selgeid reegleid. Peaaegu surid "tehisintellekti talve" ajal.',
   'Arusaam, et näidete põhjal õppimine ületab käsitsi reeglite kodeerimise, muutis kõike.',
   'Imetajad, kes elasid asteroidi üle — alguses vaiksed, nüüd valitsevad planeeti.'),

  ('era-transformers', 'et',
   'Transformerid',
   'Revolutsiooniline arhitektuur, mis töötleb kõiki sõnu samaaegselt "tähelepanu" abil — võimaldades enneolematut keelemõistmist.',
   'Kõige olulisem arhitektuuriline uuendus kaasaegses tehisintellektis. Iga suur keelemudel on Transformer.',
  ...
  ('eliza', 'et',
   ...
   'Näitas, et isegi lihtsad reeglid võivad luua intelligentsuse illusiooni.',
   'Esimene kala, kes kõndis maal — primitiivne, kuid murranguline.'),

  ('expert-systems', 'et',
   'Eksperdisüsteemid',
   'Programmid, mis kodeerisid inimeksperdi teadmisi KUI-SIIS reeglitena. Domineerisid 1980ndatel.',
   'Esimene ärilisel turul edukas tehisintellekt — kuni nad jõudsid "teadmiste pudelikaelani".',
   'Trilobiidid — ideaalselt kohandunud oma ajastule, katastroofiliselt võimetud järgmiseks.'),

  ('prolog', 'et',
   'Prolog / Loogiline programmeerimine',
   'Paradigma, kus deklareerid, mida soovid, mitte kuidas arvutada. Masin arutleb eesmärgist tagurpidi.',
   'Mõjutas formaalset verifitseerimist ja teadmiste esitamist, mis toetab kaasaegset tehisintellekti arutlust.',
   'Tsilakant — arvati olevat välja surnud, kuid elab endiselt nišikeskkondades.'),

  -- Neural
  ('perceptron', 'et',
   'Pertseptron',
   'Lihtsaim närvivõrk: üks kunstlik neuron, mis õpib sisendeid klassifitseerima kaalude kohandamise teel.',
   'Seeme, millest lõpuks kasvas süvaõpe. Selle esialgne läbikukkumine lükkas tehisintellekti aastakümneteks tagasi.',
   'Üherakuline organism — uskumatult lihtne, kuid sisaldab iga järgnenud närvivõrgu DNAd.'),

  ('backpropagation', 'et',
   'Tagasilevitamine',
   'Algoritm, mis õpetab närvivõrkudele: võrdleb väljundit soovitud tulemusega, seejärel kohandab kaalusid võrgus tagasi.',
   'Ilma tagasilevitamiseta on süvaõpe võimatu. See on mootor, mis toidab iga kaasaegse närvivõrgu treeningut.',
   'Fotosüntees närvivõrkude jaoks — fundamentaalne protsess, mis laseb neil kasvada.'),

  ('cnn', 'et',
   'Konvolutsiooniline närvivõrk (CNN)',
   'Piltidele spetsialiseerunud närvivõrk: skannib väikseid tükke õpitud filtritega, ehitades servadest objektideni.',
   'Revolutsioonistas kompuuternägemist. Tegi võimalikuks näotuvastuse, isejuhtivad autod ja meditsiinilise pilditöötluse.',
   'Silmade evolutsioon — ühtäkki masinad hakkasid nägema.'),

  ('rnn', 'et',
   'Rekurrentne närvivõrk (RNN)',
   'Mäluga närvivõrk: töötleb jadasid, suunates oma väljundi tagasi sisendiks, säilitades varjatud olekut.',
   'Esimene arhitektuur, mis suutis käsitleda jadasid. Kaasaegsete keelemudelite eelkäija.',
   'Esimene olend lühimäluga — revolutsiooniline, kuid kipub unustama.'),

  ('lstm', 'et',
   'LSTM',
   'Täiustatud RNN "väravatega", mis kontrollivad, mida meeles pidada ja mida unustada.',
   'Võimaldas praktilist jadatöötlust: masintõlge, kõnetuvastus, teksti genereerimine enne Transformereid.',
   'Elav fossiil — kunagi jadamodelleerimise tippkiskja, nüüd väljatõrjutud, kuid leidub pärandsüsteemides.'),

  ('gan', 'et',
   'Generatiivne adversatiivne võrk (GAN)',
   'Kaks närvivõrku võistluses: Generaator loob võltsinguid, Diskriminaator tuvastab neid. Mõlemad paranevad.',
   'Avas tee tehisintellekti poolt loodud piltidele. Deepfake''id ja varane tehisintellektikunst tulid GANidest.',
   'Evolutsioon läbi võistluse — kiskja ja saakloom kiirendavad teineteise arengut.'),

  -- Transformers
  ('attention-paper', 'et',
   'Attention Is All You Need',
   '2017. aasta Google Brain artikkel, mis tutvustas Transformeri arhitektuuri, asendades korduvuse paralleelse enesetähelepanuga.',
   'Kõige viidatum ja mõjukam tehisintellekti artikkel ajaloos. Tegi võimalikuks GPT, BERT, Claude ja kõik kaasaegsed keelemudelid.',
   'Tehisintellekti "K-T asteroid" — mutatsioon, mis muutis kõike.'),

  ('bert', 'et',
   'BERT',
   'Google''i kahesuunaline Transformer: loeb teksti mõlemast suunast samaaegselt, mõistmaks sügavalt konteksti.',
   'Muutis otsingumootorid ja NLP võrdlusnäitajad. Näitas, et suurel tekstimassil eelkoolitus toimib.',
   'Herbivoor Transformer — loeb ja mõistab, kuid ei genereeri.'),

  ('gpt-1', 'et',
   'GPT-1',
   'OpenAI esimene Generative Pre-trained Transformer (2018): näitas, et toorele tekstile eelkoolitus loob kasuliku keelemõistmise.',
   'Tõestas "eelkooli, siis hääleta" paradigmat. Kaasaegsete standardite järgi väike (117M parameetrit).',
   'Esimene hominiid — väikese ajuga, kuid kõndis püsti.'),

  ('gpt-2', 'et',
   'GPT-2',
   'Mudel, mis oli "liiga ohtlik avaldamiseks" (1.5B parameetrit). Genereeris üllatavalt sidusat teksti.',
   'Demonstreeris teksti genereerimise võimalusi mastaabis. Esimene mudel, mis käivitas avaliku arutelu tehisintellekti riskide üle.',
   'Homo erectus — omandas tule (sidus tekst) ja hoiatas teisi liike.'),

  ('gpt-3', 'et',
   'GPT-3',
   '175 miljardit parameetrit. Kirjutas esseesid, koodi, luulet ja tõlkis — ilma ühegi ülesandesuunitlusega treeninguta.',
   'Tõestas "skaleerimisseadused" — suuremad mudelid on kvalitatiivselt erinevad. Käivitas alusmudeli ajastu.',
   'Homo sapiens — mitte suurim, kuid võimekaim, muutes kogu maastikku.'),

  ('dall-e', 'et',
   'DALL-E',
   'OpenAI tekstist pildiks mudel: genereerib pilte loomuliku keele kirjeldustest muudetud GPT arhitektuuri abil.',
   'Näitas, et Transformerid võivad ühendada modaalsusi — mõistes teksti piisavalt, et luua vastavaid pilte.',
   'Esimene kahepaikne — Transformer, mis õppis hingama uues keskkonnas (pildid).'),

  ('gpt-4', 'et',
   'GPT-4',
   'Multimodaalne mudel, mis vastab tekstile ja piltidele. Läbib advokatuuri eksami, scores tipus.',
   'Tõi tehisintellekti võimalused tasemele, mis sundis igat tööstust uuesti mõtlema oma suhet tehnoloogiaga.',
   'Kaasaegne tsivilisatsioon — mitte ainult intelligentne, vaid suuteline arutlema üle valdkondade.'),

  ('claude', 'et',
   'Claude',
   'Anthropicu Konstitutsioonilise tehisintellekti assistent, loodud ohutuse ja kasulikkuse kui põhimõtetega.',
   'Tutvustas "Konstitutsioonilist tehisintellekti" — treenis tehisintellekti olema kasulik, kahjutu ja aus.',
   'Diplomaatlik liik — arenenud olema võimas JA koostööaldis.'),

  ('gemini', 'et',
   'Gemini',
   'Google DeepMind''i multimodaalne mudelite perekond, mis mõistab teksti, pilte, heli ja videot ühes arhitektuuris.',
   'Esindab Google''i aastakümnete tehisintellekti uurimise koondamist ühtseks mudelite perekonnaks.',
   'Kujumuutja — võrdselt mugav töötlema teksti, pilte, heli ja koodi.'),

  ('llama', 'et',
   'LLaMA',
   'Meta avatud kaaludega keelemudelite perekond. Avalikult välja antud, võimaldades globaalset peenhäälestatud variantide ökosüsteemi.',
   'Demokratiseeris suured keelemudelid. Näitas, et avatud lähtekoodiga tehisintellekt võib võistelda omandmudelitega.',
   'Avatud lähtekoodiga liik — põgenes laborist ja edenes looduses.'),

  ('diffusion', 'et',
   'Difusioonimudelid',
   'Arhitektuur, mis genereerib pilte, õppides ümber pöörama müra lisamise protsessi — alustades mürast ja luues järk-järgult selgust.',
   'Ületas GANid pildikvaliteedis. Toidab praegust revolutsiooni tehisintellekti poolt loodud kunstis ja videos.',
   'Täiesti erinev puu haru — konvergentne evolutsioon, mis loob silmapaistvat visuaalset intelligentsust.'),

  ('stable-diffusion', 'et',
   'Stable Diffusion',
   'Stability AI avatud lähtekoodiga pildigenereerimise mudel, mis kasutab latentset difusiooni. Töötab tarbija riistvaral.',
   'Tegi tehisintellektikunsti genereerimise kõigile kättesaadavaks. Käivitas massiivsed kultuurilised ja õiguslikud arutelud.',
   'Võilill — levitab seemneid igale poole, kasvab igas aias.'),

  ('midjourney', 'et',
   'Midjourney',
   'Sõltumatu tehisintellektikunsti generaator, tuntud eriti esteetilise, kunstilise väljundi poolest. Ligipääsetav Discordi kaudu.',
   'Tõestas, et tehisintellekt suudab luua tõeliselt ilusat kunsti, võites võistlusi ja muutes loovtööstusi.',
